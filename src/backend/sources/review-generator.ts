import type { GeneratableProduct, RawReview } from "@/models";
import {
  COUNTRY_WEIGHTS,
  FIRST_NAMES,
  GENERATOR_BASE_ISO,
...
const BASE_MS = Date.parse(GENERATOR_BASE_ISO);
const DAY_MS = 86_400_000;

const weightedRating = (rng: () => number): number => {
...
const weightedCountry = (rng: () => number): string => {
  const total = COUNTRY_WEIGHTS.reduce((s, c) => s + c.weight, 0);
  let roll = rng() * total;
  for (const { name, weight } of COUNTRY_WEIGHTS) {
...
const maybeAuthor = (rng: () => number): string | null => {
  const roll = rng();
...
const poolFor = (rating: number) => {
  if (rating >= 4) return POSITIVE;
  if (rating === 3) return NEUTRAL;
  return NEGATIVE;
};

const buildBody = (rng: () => number, rating: number): string => {
  const pool = poolFor(rating);
  return `${pick(rng, pool.openers)} ${pick(rng, pool.details)} ${pick(rng, pool.closers)}`;
};

const buildTitle = (rng: () => number, rating: number): string | null => {
...
const formatDate = (rng: () => number, ms: number): Partial<RawReview> => {
  const roll = rng();
